...
namespace MCTS
{
    public class MCTSAsset
    {
        ...
        private bool MctsInProgress = false;
        ...
        private IEnumerable<DynamicPropertyResult> MCTSSearch(IQueryContext context, Name actionVar, Name targetVar)
        {
            ...
            if (this.ToDoActionsList.Count == 0 && MctsInProgress ==false)
            {
                ...
                foreach (var action in worldModel.GetExecutableActions())
                {
                ...
                foreach (var item in worldModel.inventoryList)
                {
                    Console.WriteLine("Item id: " + item.Key + " | Item Quantity: " + item.Value.quantity);
                }

                foreach (var item in worldModel.Walter.Inventory)
                {
                    Console.WriteLine("Resource in inv name: " + item.Key + " | Resource in inv quantity: " + item.Value);
                }

                MctsInProgress = this.Mcts.InProgress;
                Console.WriteLine("action type: " + MacroAction.GetType());
                this.ToDoActionsList = MacroAction.Decompose(worldModel);
                Console.WriteLine("ToDoActionList Lenght: " + ToDoActionsList.Count);
